...
void LifeCycle::PreparingTexturesView3()
{
	...
	m_vecSlotFigures =
	{	Slot.GetFigure1(),Slot.GetFigure2(),Slot.GetFigure3(),
		Slot.GetFigure4(),Slot.GetFigure5(),Slot.GetFigure6(),
		Slot.GetFigure7(),Slot.GetFigure8(),Slot.GetFigure9()};
...
void LifeCycle::CashOutAnimation()
{
	...
	// casting to string / WINDOWS

	stringstream ss;
	ss << m_nCredit;
	string creditString = ss.str();

	string finalCreditText = "You Have Win " + creditString + " BGN";
	...
	while (animationFinished == false)
	{
		...
		if (counterTextures > 91)
		{
			...
			while (m_nCredit > 0) // Effect Counting Down Credit
			{
				...
				finalCreditText = "You Have Win " + creditString + " BGN";

				finalCredit = TTF_RenderText_Solid(Xanadu,
						finalCreditText.c_str(), black);

				finalCreditTexture = SDL_CreateTextureFromSurface(rendererPtr,
						finalCredit);
...
void LifeCycle::PresentView3()
{
	if (counterViewControllers == 3)
	{
		...
		// casting to int from string / WINDOWS

		stringstream ss;
		ss << totalBetCalc;
		string totalBetCalcString = ss.str();
		stringstream pp;
		pp << m_nLinesCounter;
		string linesCounterString = pp.str();

		int betTextInt = atoi(betText.c_str());

		totalBetCalc = m_nLinesCounter * betTextInt;

		totalBetText = totalBetCalcString;

		linesText = linesCounterString;
		...
		totalBetSurface = TTF_RenderText_Solid(Xanadu, totalBetText.c_str(),
				color);
		...
		tt << m_nCredit;
		string m_nCredtiString = tt.str();

		creditText = m_nCredtiString;
		...
		{
			SDL_RenderCopy(rendererPtr, m_vecLines[m_nLinesCounter], NULL,
					&Rect.rectSlot);
		}
...
//        creditText = to_string(m_nCredit);
		...
		stringstream ww;
		ww << m_nCredit;
		string m_nCreditString = ww.str();

		creditText = m_nCreditString;
...
void LifeCycle::CreditMenuZoomOut(bool okPushed)
{
	if (okPushed == true)
	{
		...
		// Creating new xml file for the next result in the game

		introController.PlayNewGame();
		//saving credits for the next stage in the game
...
void LifeCycle::SpinningReels() // Effect Spinning REELS
{
	...
	// LOGIC -> setting random figures on reel

	gameController.Spin();
	gameController.GetGameReels();
	gameController.PrintReels();
			...
			&& Rect.rectFigure1Slot5.y <= 520) // Effect SPINNING REELS
	{
		...
		if (spinRolls == 9)
		{
			...
			SDL_RenderCopy(rendererPtr,
					m_vecSlotFigures[gameController.GetGameReels()[0][0]], NULL,
					&Rect.rectFigure1Slot1); // Fig 1  // Slot 1

			SDL_RenderCopy(rendererPtr,
					m_vecSlotFigures[gameController.GetGameReels()[1][0]], NULL,
					&Rect.rectFigure2Slot1); // Fig 2

			SDL_RenderCopy(rendererPtr,
					m_vecSlotFigures[gameController.GetGameReels()[2][0]], NULL,
					&Rect.rectFigure3Slot1); // Fig 3
		}
		...
		if (spinRolls2 > 11)
		{
			...
			SDL_RenderCopy(rendererPtr,
					m_vecSlotFigures[gameController.GetGameReels()[0][1]], NULL,
					&Rect.rectFigure1Slot2); // Fig 1  // Slot 2

			SDL_RenderCopy(rendererPtr,
					m_vecSlotFigures[gameController.GetGameReels()[1][1]], NULL,
					&Rect.rectFigure2Slot2); // Fig 2

			SDL_RenderCopy(rendererPtr,
					m_vecSlotFigures[gameController.GetGameReels()[2][1]], NULL,
					&Rect.rectFigure3Slot2); // Fig 3
		...
		if (spinRolls3 > 13)
		{
			...
			SDL_RenderCopy(rendererPtr,
					m_vecSlotFigures[gameController.GetGameReels()[0][2]], NULL,
					&Rect.rectFigure1Slot3); // Fig 1  // Slot 3

			SDL_RenderCopy(rendererPtr,
					m_vecSlotFigures[gameController.GetGameReels()[1][2]], NULL,
					&Rect.rectFigure2Slot3); // Fig 2

			SDL_RenderCopy(rendererPtr,
					m_vecSlotFigures[gameController.GetGameReels()[2][2]], NULL,
					&Rect.rectFigure3Slot3); // Fig 3
		...
		if (spinRolls4 > 15)
		{
			...
			SDL_RenderCopy(rendererPtr,
					m_vecSlotFigures[gameController.GetGameReels()[0][3]], NULL,
					&Rect.rectFigure1Slot4); // Fig 1  // Slot 4

			SDL_RenderCopy(rendererPtr,
					m_vecSlotFigures[gameController.GetGameReels()[1][3]], NULL,
					&Rect.rectFigure2Slot4); // Fig 2

			SDL_RenderCopy(rendererPtr,
					m_vecSlotFigures[gameController.GetGameReels()[2][3]], NULL,
					&Rect.rectFigure3Slot4); // Fig 3
		...
		if (spinRolls5 > 17)
		{
			...
			SDL_RenderCopy(rendererPtr,
					m_vecSlotFigures[gameController.GetGameReels()[0][4]], NULL,
					&Rect.rectFigure1Slot5); // Fig 1  // Slot 5

			SDL_RenderCopy(rendererPtr,
					m_vecSlotFigures[gameController.GetGameReels()[1][4]], NULL,
					&Rect.rectFigure2Slot5); // Fig 2

			SDL_RenderCopy(rendererPtr,
					m_vecSlotFigures[gameController.GetGameReels()[2][4]], NULL,
					&Rect.rectFigure3Slot5); // Fig 3
...
//    m_nCredit = m_nCredit - stoi(totalBetText);
	...
	int totalBetInt = atoi(totalBetText.c_str());

	m_nCredit = m_nCredit - totalBetInt;
...
bool LifeCycle::IsPlayPressed(int x, int y)
{
	if (ev.type == SDL_MOUSEBUTTONDOWN)
	{
				...
				&& counterViewControllers == 2)
		{
			...
			// setting default view to the game
...
bool LifeCycle::IsStartSpinPressed(int x, int y) // Start Spin Button
{
	if (ev.type == SDL_MOUSEBUTTONDOWN)
	{
				...
				&& counterViewControllers == 3)
		{
			...
			gameController.TotalBetAsString();
			gameController.PrintTotalBet();
...
bool LifeCycle::IsMinusLinesPressed(int x, int y) // Minus Lines Button
{
	if (ev.type == SDL_MOUSEBUTTONDOWN)
	{
		if (xZ >= Rect.rectMinusButtonLines.x
				...
				&& counterViewControllers == 3)
		{
			// decreasing lines LOGIC

			gameController.DecreasePaylines();
			//check in cout
			gameController.PrintNumLines();
